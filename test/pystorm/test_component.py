...
class AsyncComponentTests(unittest.TestCase):
    ...
    def test_read_handshake(self):
        ...
        inputs = ["{}\n".format(json.dumps(handshake_dict)),
                  "end\n"]
        component = AsyncComponent(input_stream=BlockingBytesIO(''.join(inputs).encode('utf-8')),
    ...
    def test_read_message_unicode(self):
        ...
        outputs = [json.loads(msg) for msg in inputs[::2] if msg]
        outputs.append('')
        component = AsyncComponent(input_stream=BlockingBytesIO(''.join(inputs).encode('utf8')),
                                   output_stream=BlockingBytesIO())
        component._reader.start()
        component._writer.start()
        for output in outputs:
    ...
    def test_send_message(self):
        component = AsyncComponent(input_stream=BlockingBytesIO(), output_stream=BlockingBytesIO())
        component._reader.start()
        component._writer.start()
        inputs = [{"command": "emit", "id": 4, "stream": "", "task": 9,
                   "tuple": ["field1", 2, 3]},
        ...
        for cmd in inputs:
            component.serializer.output_stream.close()
            component.serializer.output_stream = component.serializer._wrap_stream(BlockingBytesIO())
            component.send_message(cmd)
            self.assertEqual(component.serializer.serialize_dict(cmd).encode('utf-8'),
                             component.serializer.output_stream.buffer.getvalue())
